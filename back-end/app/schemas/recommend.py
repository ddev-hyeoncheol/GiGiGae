...
class BrandRecommendRequest(BaseModel):
    ...
    brand_tone: list[str] = Field(
        default=[], description="브랜드 톤 (예: 트렌디한, 모던한)"
    )
    count: int | None = Field(
        default=None, ge=1, le=10, description="추천 개수 (미입력 시 서버 기본값)"
    )
...
class BrandRecommendCandidate(BaseModel):
    ...
    brand_description: str = Field(
        ..., description="브랜드 한 줄 설명 (예: 감성을 담은 반려동물 라이프)"
    )
    brand_tags: list[str] = Field(
        ..., description="형용사 태그 2~3개 (예: 감각적인, 친근한, 혁신적인)"
    )
